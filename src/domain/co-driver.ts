...
//
// С 16.08.2026 те же правила действуют и в цехе (решение Артёма): различается только КОГО можно
// поставить напарником — это решает контур в task-service (водитель или исполнитель цеха).
//
// Инварианты пары:
...
/**
 * Валидация напарника при создании/правке задачи (кандидат уже проверен на пригодность в контуре —
 * task-service, assertAssignableFor). Возвращает нормализованное значение или бросает.
 */
export function validateCoDriver(
  ...
  if (assigneeId === null) {
    // «Ответственный», а не «ответственный водитель»: в цехе пару собирают из сотрудников.
    throw new CoDriverRuleError("Сначала назначьте ответственного");
  }
