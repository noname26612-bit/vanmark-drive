...
//
// Доставки — 60 мин (решение Артёма 11.08.2026): к 30 минутам «на объекте» добавлены 30 минут на
// выгрузку станка, которых в норме не было. Типы у нас объединённые («Доставка / забор из …»),
// поэтому те же 60 минут действуют и на заборы этих типов — погрузка занимает столько же.
export const ON_SITE_MINUTES: Record<string, number> = {
  "Выездной ремонт / диагностика": 90,
  "Гарантийный ремонт": 90,
  "Доставка / забор из аренды": 60,
  "Доставка / забор из ремонта": 60,
  "Доставка проданного об.": 60,
  // Исторические имена доставок из прежнего набора типов (до объединения 10→8). На разных базах
  // сохранились разные написания — обновляем все, лишнее просто не найдётся (updateMany по имени).
  "Доставка проданного оборудования": 60,
  "Доставка в аренду": 60,
  "Доставка из ремонта": 60,
  "Закупка/выкуп станка": 30,
  "Сдача / забор из ТК": 60,
  Прочее: 30,
};
...
export async function seedCapacity(prisma: PrismaClient): Promise<void> {
  ...
  // Ключей в таблице больше, чем типов в базе: у «Доставки проданного» два исторических написания,
  // и найдётся ровно одно. Поэтому печатаем сколько типов реально обновили, без знаменателя.
  let normed = 0;
  ...
  console.log(`  ✓ нормы времени по типам: обновлено ${normed}`);
