...
test("детектор находит нарушение из реальной задачи, диспетчер подтверждает", async ({ browser }) => {
  ...
  const before = ov1.drivers.find((d: { driverName: string }) => d.driverName === "Алексей Каширский");

  // Подтверждаем одним действием.
  const res = await milena.request.post(`/api/kpi/marks/${cand.id}/resolve`, {
    data: { status: "CONFIRMED" },
  });
  expect(res.status()).toBe(200);

  // После подтверждения кандидат уходит из списка, штраф водителя растёт.
  ...
  const after = ov2.drivers.find((d: { driverName: string }) => d.driverName === "Алексей Каширский");
