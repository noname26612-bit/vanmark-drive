...
test("живое обновление: смена статуса водителем видна у Милены на доске ≤10 с без F5", async ({
  ...
  await dialog.locator('[data-testid="create-type"]').selectOption({ label: "Сдача в ТК" });
  ...
  // Карточка появилась на доске со статусом «Назначена».
  const card = milena.getByTestId("board-card").filter({ hasText: title });
  await expect(card).toBeVisible();
  await expect(card.getByText("Назначена")).toBeVisible();
  ...
  // Водитель в отдельном контексте принимает задачу (реальная смена статуса исполнителем).
  ...
  // На доске Милены (страница НЕ перезагружалась) статус обновляется поллингом до «Принята».
  await expect(card.getByText("Принята")).toBeVisible({ timeout: 15_000 });
  await expect(card.getByText("Назначена")).toHaveCount(0);
...
test("доска: блок «Требуют внимания» показывает пропуск на завтра и просрочку", async ({ page }) => {
  ...
  await dialog.locator('[data-testid="create-type"]').selectOption({ label: "Доставка в аренду" });
  ...
  await dialog.locator('input[type="date"]').fill(localDate(1));
