...
// O8 «обрыв в пути», разбор конфликта: водитель офлайн берёт задачу в работу, диспетчер тем временем
// отменяет её. При возврате связи досылка отклоняется сервером (переход из CANCELLED невозможен) →
// действие помечается «конфликт». Статус в UI больше не «висит» IN_PROGRESS (виден серверный арбитр),
// водитель разбирает конфликт кнопкой «Убрать», и очередь очищается.
test("офлайн: отклонённое при досылке действие → конфликт, водитель убирает его", async ({ browser }) => {
  test.slow();
  const mctx = await browser.newContext();
  const milena = await mctx.newPage();
  await login(milena, "milena");
  ...
  // Офлайн: берём задачу в работу — оптимистичный оверлей «В работе».
  await dctx.setOffline(true);
  await driver.getByRole("button", { name: "В работу" }).click();
  await expect(driver.getByText("В работе").first()).toBeVisible();

  // Пока водитель офлайн, диспетчер отменяет задачу (причина обязательна).
  const cancel = await milena.request.post(`/api/tasks/${id}/transition`, {
    data: { toStatus: "CANCELLED", reason: "e2e: отменено пока водитель офлайн" },
  });
  expect(cancel.status()).toBe(200);

  // Связь вернулась → досылка IN_PROGRESS отклоняется (задача уже CANCELLED) → баннер конфликта.
  await dctx.setOffline(false);
  await expect(driver.getByTestId("conflict-banner")).toBeVisible({ timeout: 20_000 });

  // Статус на сервере остался CANCELLED (конфликтное действие не применилось, идемпотентность цела).
  expect((await (await milena.request.get(`/api/tasks/${id}`)).json()).data.status).toBe("CANCELLED");

  // Водитель разбирает: открывает список и убирает непрошедшее действие.
  await driver.getByTestId("conflict-banner").click();
  await expect(driver.getByTestId("conflict-sheet")).toBeVisible();
  await driver.getByTestId("conflict-discard").first().click();

  // Очередь очищена — баннер конфликта исчез.
