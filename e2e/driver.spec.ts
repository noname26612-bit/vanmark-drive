...
test("водитель проходит цепочку статусов с телефона (360×740), гео-метка пишется", async ({
  ...
  const { id, title } = await createAssignedTask(milena, "Алексей Каширский", "Отвезти в ТК");
  ...
  // «Выполнено» открывает экран завершения; для типа без обязательного фото — сразу «Завершить»
...
test("изоляция: водитель A не видит и не меняет задачу водителя B", async ({ browser }) => {
  ...
  const milena = await mctx.newPage();
  await login(milena, "milena");
  const { id } = await createAssignedTask(milena, "Алексей Писарев", "Доставка в аренду");
