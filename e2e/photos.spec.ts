...
test("фото по желанию (360×740): без фото завершается сразу; фото можно приложить", async ({
  ...
  // доводим до «В работе» (этап A: схлопнутая цепочка)
  ...
  await expect(driver.getByRole("button", { name: "Завершить", exact: true })).toBeEnabled();
...
test("фото и акт НЕ блокируют DONE на сервере (любой тип завершается без вложений)", async ({
  ...
  // тип с актом «Выездной ремонт / диагностика»: DONE без фото и без акта — проходит, но водитель
  // обязан указать причину отсутствия акта (акты до 20:00, 02.07); без причины — 422.
  const repair = await createAssignedTask(milena, "Алексей Каширский", "Выездной ремонт / диагностика");
  await advanceToInProgress(driver.request, repair);
  const noReason = await driver.request.post(`/api/tasks/${repair}/transition`, {
    data: { toStatus: "DONE" },
  });
  ...
  // тип без акта «Сдача / забор из ТК»: DONE без фото — проходит
  const tk = await createAssignedTask(milena, "Алексей Каширский", "Сдача / забор из ТК");
  await advanceToInProgress(driver.request, tk);
...
test("оплата на месте: DONE без подтверждения → PAYMENT_REQUIRED; с подтверждением → отметка в истории", async ({
  ...
  await advanceToInProgress(driver.request, id);
...
test("изоляция файлов: чужой водитель → 404, гость → 401, владелец и диспетчер → 200", async ({
  ...
  const id = await createAssignedTask(milena, "Алексей Каширский", "Доставка / забор из аренды");
