...
test("водитель завершает задачу с фото (360×740): без фото нельзя, с фото — DONE", async ({
  browser,
}) => {
  test.slow();
  ...
  const id = await createAssignedTask(milena, "Алексей Каширский", "Доставка в аренду"); // requiresPhoto
  ...
  // открываем экран завершения — без фото «Завершить» недоступно
  await driver.getByRole("button", { name: "Выполнено" }).click();
  await expect(driver.getByRole("button", { name: "Завершить" })).toBeDisabled();

  // добавляем фото с «камеры»
  await driver.locator('input[type="file"]').setInputFiles({
    name: "photo.jpg",
    mimeType: "image/jpeg",
    buffer: JPEG,
  });
  // фото появилось → «Завершить» активна
  ...
  // Оптимистика показывает статус сразу — POST транзишна летит следом. Ждём подтверждения сервера.
  await expect
    .poll(
      async () => {
...
test("гейт фото на сервере: requiresPhoto без фото → PHOTO_REQUIRED; без requiresPhoto → DONE", async ({
  browser,
}) => {
  test.slow();
  const mctx = await browser.newContext();
  ...
  // requiresPhoto: «Доставка в аренду» — DONE без фото запрещён
  const withPhoto = await createAssignedTask(milena, "Алексей Каширский", "Доставка в аренду");
  await advanceToOnSite(driver.request, withPhoto);
  const blocked = await driver.request.post(`/api/tasks/${withPhoto}/transition`, {
    data: { toStatus: "DONE" },
  });
  expect(blocked.status()).toBe(422);
  expect((await blocked.json()).error.code).toBe("PHOTO_REQUIRED");
  const stillOnSite = await milena.request.get(`/api/tasks/${withPhoto}`);
  expect((await stillOnSite.json()).data.status).toBe("ON_SITE");

  // тот же тип, но теперь с фото от исполнителя — DONE проходит
  const up = await driver.request.post(`/api/tasks/${withPhoto}/attachments`, {
    multipart: { file: { name: "p.jpg", mimeType: "image/jpeg", buffer: JPEG } },
  });
  expect(up.status()).toBe(201);
  const doneWithPhoto = await driver.request.post(`/api/tasks/${withPhoto}/transition`, {
    data: { toStatus: "DONE" },
  });
  expect(doneWithPhoto.status()).toBe(200);
  expect((await doneWithPhoto.json()).data.status).toBe("DONE");

  // не requiresPhoto: «Отвезти в ТК» — DONE без фото проходит
  const noPhoto = await createAssignedTask(milena, "Алексей Каширский", "Отвезти в ТК");
  await advanceToOnSite(driver.request, noPhoto);
  const okDone = await driver.request.post(`/api/tasks/${noPhoto}/transition`, {
    data: { toStatus: "DONE" },
  });
  expect(okDone.status()).toBe(200);
  expect((await okDone.json()).data.status).toBe("DONE");
...
test("оплата на месте: DONE без подтверждения → PAYMENT_REQUIRED; с подтверждением → отметка в истории", async ({
  ...
  // не requiresPhoto, чтобы изолировать гейт оплаты; ставим оплату «на месте» правкой задачи
  const id = await createAssignedTask(milena, "Алексей Каширский", "Отвезти в ТК");
