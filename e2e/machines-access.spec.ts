...
// «Команда» (18.08.2026) сюда же: сначала на просмотр, с 21.08.2026 — с правом вести записи.
const ALLOWED_PAGES = ["/board", "/planning", "/capacity", "/tasks", "/staff", "/team"];
...
const FORBIDDEN_API: [string, string][] = [
  ...
  ["POST", "/api/idle-notes"],
  // POST /api/absences отсюда УБРАН 21.08.2026: отпуска — часть вкладки «Команда», и Артём открыл
  // её менеджеру-сервиснику. Право проверяется теперь положительно, в team.spec.ts. Держать запись
  // здесь было бы самообманом: с телом {} ручка и так отвечает 400, и «не течёт» доказывалось бы
  // валидацией, а не гейтом.
...
const ALLOWED_API: string[] = [
  ...
  // Справочник коллектива (21.08.2026): он его и смотрит, и ведёт — правки проверяет team.spec.ts.
  "/api/team",
];
...
const MACHINE_API: [string, string][] = [
  ...
  // Пикер станков для формы заявки (21.08.2026) — тот же гейт картотеки.
  ["GET", "/api/machines/picker?family=BENDER"],
