...
test("акт прикладывается на ремонтной задаче и НЕ блокирует завершение", async ({ browser }) => {
  ...
  const doneNoAkt = await driver.request.post(`/api/tasks/${a}/transition`, { data: { toStatus: "DONE" } });
  expect(doneNoAkt.status()).toBe(200); // отсутствие акта НЕ блокирует завершение
