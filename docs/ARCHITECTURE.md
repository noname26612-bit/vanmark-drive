...
- **Разбор конфликтов и сбои сессии** (O8, `runQueueOnce`/`ConflictCenter`/`auth-required.ts`): конфликтные действия водитель разбирает сам — баннер «Не прошло — разобрать» → шторка с причинами → кнопка «Убрать» (`discardAction` чистит очередь и связанный blob; «Повторить» не делаем — нужное действие проще выполнить заново, решение Артёма 02.07). `overlayStatus` учитывает только pending/syncing, поэтому отклонённый переход больше не искажает статус. 401/403 при досылке — не конфликт: очередь останавливается, поднимается флаг `authRequired` (баннер «войдите заново»), после входа первое успешное действие снимает флаг и досылка продолжается. Потерянный blob фото (эвикция IndexedDB) → доменная ошибка `BLOB_MISSING` (конфликт с причиной), а не тихий успех; `navigator.storage.persist()` в `OfflineSync` снижает риск эвикции. Порядок досылки строго FIFO: монотонный `nextSeq` + тай-брейк по id.
