...
| PATCH /api/shifts/:id {op:reopen} · {op:close, closedAtTime?, reason?} · {openedAtTime\|closedAtTime, reason} | Д | переоткрыть / закрыть за водителя (№2) / правка времени открытия-закрытия задним числом (№3); работа по shiftId, личность из сессии |
