...
model User {
  ...
  canLogin      Boolean  @default(true)   // false — вход запрещён; включает админ («Пользователи и доступ»)
...
Учёт времени (этап D): отработано = Σ длительностей задач «В работе → Завершено», простой = длина закрытых смен − отработано — считается в `summary-service` и показывается в «Сводке»/CSV. KPI-изменения — см. §4а. История смен за период с правкой времени открытия/закрытия — раздел «История смен»; с 06.07.2026 он на экране «KPI / Зарплата», а не в «Сводке» (№3; уточнено 22.08.2026).
...
```prisma
enum MachineCategory { CLIENT OUR_SALE OUR_RENTAL SHOWROOM KNIFE_SETUP } // чей станок и зачем он у нас; SHOWROOM/KNIFE_SETUP — 21.08.2026, обе эксклюзивные
...
// ── Этап 2 модуля (21.08.2026): связь заявок со станками, PRD §16.9 ──────────────────────────
// Что делать со станком, когда заявку этого типа завершат. Колонка на TaskType, а НЕ матчинг по
// названию: имя типа правит админ в /admin/task-types, и правило на строке молча отвалилось бы.
enum MachineFlow          { NONE SOLD_DELIVERY RENTAL REPAIR_RETURN PURCHASE CARRIER }
enum TaskMachineDirection { OUT IN }                   // «Везём клиенту» / «Забираем к нам»

// Join-таблица, а не Task.machineId из старого эскиза: станков в заявке бывает несколько (решение
// Артёма), и на самой связи живут направление и отметки сработавшей автоматики.
model TaskMachine {
  id            String               @id @default(uuid())
  taskId        String
  machineId     String
  direction     TaskMachineDirection @default(OUT)     // у однонаправленных типов сервер ставит принудительно
  appliedStatus MachineStatus?                          // что применила автоматика…
  appliedAt     DateTime?                               // …и когда: гард идемпотентности (обрабатываем при null)
  createdById   String
  createdAt     DateTime             @default(now())
  @@unique([taskId, machineId])
  @@index([machineId, createdAt])
}
// TaskType += machineFlow MachineFlow @default(NONE); Task += machines TaskMachine[]; Machine += taskLinks TaskMachine[]
```

**Связь заявок со станками** (`src/domain/task-machine-flow.ts` — чистые правила + юнит-тесты, `src/domain/task-machine-service.ts` — работа с БД):

- **Таблица автоматики** (`flowEffect(flow, direction)`) — единственная правда о том, что делает завершение заявки со станком; вся она покрыта юнит-тестами. Продуктовая формулировка — PRD §16.9.
- **Автоматика запускается ПОСЛЕ коммита DONE**, вне транзакции задачи, отдельной короткой транзакцией на каждый станок, вся в `try/catch` (`applyMachineAutomationAfterDone`, вызов рядом с концом `transitionTask`). Три причины: (а) требование Артёма «никогда не блокировать водителя» выполнимо только когда DONE уже зафиксирован; (б) проглоченная ошибка внутри транзакции оставила бы частичные записи; (в) все пути DONE (водитель, диспетчер, офлайн-досылка) идут через `transitionTask`, а `withIdempotency` кэширует ответ — повтор из очереди автоматику не перезапускает. Окно «сервер упал между коммитом и автоматикой» принято как риск: пользователей трое, журнал показывает всё, диспетчер дожмёт руками.
- **Идемпотентность** — `appliedAt`: связь обрабатывается только при `null`, успех ставит отметку в той же транзакции, что и записи по станку. Откат из DONE отметку НЕ снимает, поэтому повторное завершение не задваивает продажу и списание склада (плюс `MachineKitPart.consumedAt` страхует склад отдельно).
- **Комплект едет всегда**: переиспользуется `loadKitForTransfer` + вынесенный из `changeStatus` помощник `applyStatusWithKitTx` (машина ручной смены состояния и автоматика ходят одним кодом — две копии неизбежно разошлись бы, а списание склада ошибок не прощает).
- **Несовпадение валидации** (состояние не подходит категориям станка, комплект не переводится) → заметки в журнал станка и в историю заявки БЕЗ `appliedAt`: повторное завершение после правки категорий добьёт.
- **Полный набор атомарно**: `setTaskMachinesTx` диффит связи против переданного набора (семантика `changeCategories`), не пересоздавая их — иначе сбросился бы `appliedAt`. Каждое изменение пишется в ОБА журнала (`TaskEvent` `machine_link`/`machine_unlink`/`machine_auto`, `MachineEvent` `task_link`/`task_unlink`).
- **Связи в списочном `taskInclude`, а не только в карточке**: форма редактирования получает задачу из списка и шлёт полный набор — без связей в списке правка с доски молча стёрла бы привязки. Select мини (номер, модель, раздел, вид) и **без цены**: этот ответ уходит в телефон водителя.
- **Задачам сотрудникам станки не привязываются**: `machines` в `DELIVERY_ONLY_FIELDS`, `stripDeliveryOnlyFields` режет их и при создании, и при PATCH.
...
- **Категории — список** (20.08.2026, расширен 21.08.2026). Инварианты держит домен, а не БД (это правила предметной области, они меняются вместе с продуктом): набор непустой; **эксклюзивная категория стоит в наборе одна**. Эксклюзивные перечислены множеством `EXCLUSIVE_CATEGORIES = {CLIENT, SHOWROOM, KNIFE_SETUP}` — единственная правда об эксклюзивности: её же читает чистая `toggleCategory(current, clicked)`, на которую переведены галочки формы (второй копии правил в UI нет). Функции: `normalizeCategories`, `isValidCategorySet`, `isExclusiveCategory`, `toggleCategory`, `isOurCategories`, `categoriesLabel`.
- «Наше» железо определяется отсутствием `CLIENT`, поэтому `SHOWROOM`/`KNIFE_SETUP` нумеруются схемой «77-N» без единой правки в `machine-number.ts`.
- Проверяется **совместимость категорий и состояния**: `RENTED` требует `OUR_RENTAL` в наборе, `SOLD` — `OUR_SALE`, `RELEASED` — `CLIENT`. Смена категорий валидируется тем же предикатом. `categoriesFollowingStatus` для неэксклюзивных наших категорий ДОБАВЛЯЕТ недостающую (сдали в аренду то, что стояло на продажу, — станок теперь и продаётся, и сдаётся), а для эксклюзивной нашей — **ЗАМЕНЯЕТ** её целиком (`[SHOWROOM] + SOLD → [OUR_SALE]`): добавить рядом нельзя по определению эксклюзивности, а запрещать продажу выставочного станка значило бы заставлять переключать категорию руками перед каждой сделкой. Переезд пишется в журнал строкой «Категории: было → стало» (`applyStatusChangeTx`).
...
- **Права роли SERVICE_MANAGER: белые списки по контурам (11.08.2026, дополнено 21.08.2026).** Роль перестала быть «только станки»: Артём открыл ей ЗАЯВКИ, а затем и СПРАВОЧНИК КОЛЛЕКТИВА, оставив закрытыми смены, KPI/нарушения, пометки простоя, «Сводку», расценку и админку. Одного предиката для этого мало, поэтому права расщеплены по контурам:
  ...
  - `isTeamManagerRole` (`src/domain/team-access.ts`, 21.08.2026) = ADMIN | DISPATCHER | SERVICE_MANAGER — вкладка «Команда»: весь `/api/team` (чтение и запись) и запись в реестр отсутствий (`POST /api/absences`, `DELETE /api/absences/:id`), guard `requireTeamManager()`. Список ЗАВЕДЁН ОТДЕЛЬНЫМ, хотя сегодня совпадает с `isTaskManagerRole` роль в роль: кадры и заявки — разные вопросы, и роль, которая завтра получит одно без другого, не должна получать второе молча. Гейт продублирован в домене (`assertTeamManager` в `team-service.ts` и `absence-service.ts`): у экрана нет изоляции «по владельцу», и право — единственная защита записи.
...
- **Роль SERVICE_MANAGER (05.08.2026, §4г).** Права роли — БЕЛЫЙ список, а не «всё кроме»: доступ к модулю станков дают три роли (`isMachineRole` = SERVICE_MANAGER | DISPATCHER | ADMIN, `src/domain/machine-access.ts`), всё остальное для новой роли закрыто по умолчанию. Ключевое требование при её вводе — **проверить существующие guard'ы на допущение «роль не DRIVER ⇒ штаб»**: `requireDispatcher` опирается на `isDispatcherRole` (белый список ADMIN|DISPATCHER — новая роль не проходит), `requireAdmin`/`requireDriver` — точное сравнение, страницы — `requireRole`/`requireAnyRole` с редиректом на `homeForRole`. Отдельно проверяются эндпоинты с одним лишь `requireApiUser()` (без роли): `canViewTask` для SERVICE_MANAGER возвращает false → чужая карточка/вложение отдают 404. Маршруты новой роли: `homeForRole('SERVICE_MANAGER') = '/machines'`; `exhaustive switch` в `src/domain/roles.ts` заставляет типизацию упасть, если роль забыли учесть.
  ...
  - Модуль станков изоляции «по владельцу» не имеет by design (все три роли видят весь парк — это общая картотека, а не личные задачи), поэтому вся его защита — ролевая: `requireMachineUser()` в каждом handler'е. Водитель к любому `/api/machines/*` получает **404** (не 403 — не раскрываем существование модуля). Пикер станков для формы заявки (`GET /api/machines/picker`, 21.08.2026) — тот же гейт.
  - **Единственное исключение — раздача фото станка** (`GET /api/machines/photos/:id`, 21.08.2026): водитель видит в телефоне фото станка, который везёт, значит и файл ему надо отдать. Гейт не ослаблен, а перенесён в домен (`getMachinePhotoForViewer`): дверей ровно две — (1) роль из белого списка ИЛИ `equipmentAccess` из БД; (2) фото принадлежит станку, привязанному к **активной** (не архивной) заявке, где смотрящий — ответственный или напарник. Всё остальное — **404**, включая несуществующее вложение: сначала находим запись, потом проверяем права, иначе разные коды сами отвечали бы на вопрос «а есть ли такой снимок». `DELETE` того же пути остался на `requireMachineUser()`. Покрыто e2e (`task-machines.spec.ts`: исполнителю 200, чужому водителю 404, после архивации заявки 404).
  - **Автоматика по станку при завершении заявки** (`applyMachineAutomationAfterDone`) `assertMachineAccess` не вызывает — и это осознанно: карточку двигает не человек, а система, по связи, которую поставил допущенный диспетчер, и по правилу, которое задал админ. Водитель картотеку по-прежнему не видит и напрямую в неё не пишет; его единственный вход сюда — завершение СВОЕЙ задачи (изоляция проверена в `transitionTask` до коммита), а `actor` нужен лишь чтобы подписать события журналов настоящим автором. Привязку же связей (`resolveTaskMachines`) гейт картотеки проверяет ЯВНО, хотя вызывающий уже проверил право вести заявки: сегодня оба белых списка совпадают роль в роль, но полагаться на это — ровно та мина, о которой предупреждает этот раздел.
...
- **Роль `EMPLOYEE` (18.08.2026, §4д) — учётка, у которой не бывает сессии.** Сотрудника без доступа заводит `POST /api/team`, и войти под ним нельзя по трём независимым причинам сразу: `canLogin=false` (проверяется в `authorize`), пароль случайный (uuid — не известен никому и нигде не показывается), а включить вход некому — админские ручки доступа (`/api/admin/drivers`) работают только с ролями, у которых вход в принципе бывает (`MANAGED_ROLES` = ADMIN · DISPATCHER · SERVICE_MANAGER · DRIVER; с 22.08.2026 пароль и вход меняются и учёткам офиса), а `EMPLOYEE` в этот белый список не входит — для него 404. Три «водительских» признака (`isExternal`, `equipmentAccess`, `staffTasksAccess`) по-прежнему только у `role='DRIVER'`. Прав роль не даёт и сама по себе: её нет ни в одном белом списке (`isDispatcherRole`, `isTaskManagerRole`, `isMachineRole`, точное сравнение в `requireDriver`), а `equipmentAccess`/`staffTasksAccess` — отдельные флаги, до которых справочник не дотягивается. Роль, логин, вход и флаги доступа не принимаются из тела запроса вовсе: их ставит `createEmployee`, а в типе патча (`TeamMemberPatch`) таких полей физически нет — справочник коллег не должен уметь раздавать права.
- **Гейты вкладки «Команда»** (§4д, границы уточнены 21.08.2026): весь `/api/team` — и чтение, и запись — стоит на `requireTeamManager()` (Д, А, С). До 21.08.2026 запись держал `requireDispatcher()`; Артём открыл правку менеджеру-сервиснику, и ослаблять ради этого диспетчерский предикат было нельзя — вместе с кадрами он отдал бы Максиму смены, KPI и деньги, поэтому заведён отдельный белый список (`team-access.ts`). Смотрит и правит теперь один и тот же набор ролей, так что `canManage` на странице совпадает с её guard'ом и разъехаться им нечем. Изоляции «по владельцу» у экрана нет by design (как у картотеки станков — справочник общий); вместо неё две доменные границы, и они НЕ ослаблялись: у учётки С доступом сервис принимает только `birthday` (роли, логина, входа и флагов доступа нет в типе патча — эскалация невозможна), а «неживые» для экрана (уволенный, внешний перевозчик, не-`EMPLOYEE` в `deactivateEmployee`) получают **404** — то есть отключить чужую боевую учётку через справочник нельзя. У `/api/absences` чтение осталось на `requireTaskManager()` (его смотрит ещё и «Календарь загрузки» — это планирование заявок), а запись переехала на `requireTeamManager()`: отсутствия — часть «Команды». Побочный эффект зафиксирован осознанно: дни отпуска исключают авто-нарушение «невыполненная точка» (`kpi-service.ts`), значит менеджер-сервисник косвенно влияет на KPI водителя — ровно в том же объёме, в каком это могла делать Милена, и так же видно всем на «Календаре» и во вкладке.
...
| POST /api/tasks/:id/transition {toStatus, comment?, lat?, lng?, paymentConfirmed?, paymentAmount?} | по матрице | смена статуса + событие + пуш; DONE: «деньги получены» (ON_SITE) — серверный гейт. С 21.08.2026 после коммита DONE у доставки запускается автоматика по привязанным станкам (§4г, PRD §16.9) — вне транзакции и в try/catch: завершение она заблокировать не может |
...
| GET/POST /api/admin/users, /api/admin/task-types · PATCH /api/admin/task-types/:id | А | справочники. У типа задачи с 21.08.2026 есть `machineFlow` — что делать с привязанным станком при завершении заявки (PRD §16.9); служебному типу задач сотрудникам можно только `NONE` |
| GET /api/admin/drivers · PATCH {userId, canLogin\|isExternal\|equipmentAccess\|staffTasksAccess} · POST /api/admin/drivers/password | А | экран «Пользователи и доступ»: вход и пароль (03.08 — водителям, 22.08 — учёткам офиса), признак «внешний перевозчик» и персональные флаги доступа. Вход и пароль работают с `MANAGED_ROLES` (ADMIN · DISPATCHER · SERVICE_MANAGER · DRIVER), «водительские» признаки — только с DRIVER; всё остальное (EMPLOYEE, уволенный, несуществующий) → 404. Вход НЕЛЬЗЯ закрыть себе (личность из сессии) и последнему активному админу со входом (`LAST_ADMIN`, 409). Поле `driverId` принимается как устаревший синоним `userId`; пароль только в теле, не логируется и не возвращается |
...
| GET /api/summary/overview?granularity&date | Д | сводка по водителям за период (день/неделя/месяц, по дате закрытия задач) — Фаза 2, только чтение. С 22.08.2026 отдаёт ещё и `workdayMinutes` (настройка ёмкости) — фиксированная шкала графика по дням |
| GET /api/summary/export?granularity&date | Д | та же сводка файлом CSV (вложение, BOM+`;` для Excel) |
| GET /api/summary/details?metric&granularity&date&driverId? | Д | список за цифрой Сводки (задачи/смены/пометки); неизвестная метрика → 422 |
| GET /api/summary/carrier?granularity&date · GET /api/summary/carrier/export | Д | затраты на внешнего перевозчика за период и тот же отчёт файлом CSV |
| GET /api/summary/shifts?granularity&date | Д | журнал смен периода для раздела «История смен» (живёт на экране «KPI / Зарплата») |
...
| GET /api/machines/picker?family=BENDER\|SEAMER&q= | С, Д, А | компактный список станков для выбора в форме заявки (21.08.2026, PRD §16.9): номер, модель, состояние, комплектация, признак «отметки не проставлены». Без аннулированных и складских остатков (те едут в комплекте головного); архивные и «в аренде» показываются — их и забирают обратно |
| GET /api/machines/photos/:id | С, Д, А **+ В по своей заявке** | файл с проверкой прав (nosniff, не из public/). С 21.08.2026 — единственный handler модуля без `requireMachineUser`: доступ даёт либо картотека (роль/`equipmentAccess`), либо активная заявка со станком, где смотрящий ответственный или напарник; любой отказ — 404 (§6). Путь намеренно не `/api/machines/attachments/:id` — тот пересекался бы с `/api/machines/:id/...` |
